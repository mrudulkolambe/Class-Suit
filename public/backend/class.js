...
db.collection("classes").doc(window.localStorage.getItem('document'))
    .onSnapshot((doc) => {
        getData(doc)
    });
...
async function getData(doc) {
    ...
    if (users.includes(`${currentUser.email}`)) {
        ...
        window.localStorage.setItem('announcement_permission', data.announcementPermssionBoolean);
...
function removeUser(arr, user) {
    ...
    console.log(newArr)
    db.collection("classes").doc(window.localStorage.getItem('document')).update({
...
function sendAnnouncement() {
    ...
    let nowDate = getDate()
    db.collection('classes').doc(window.localStorage.getItem('document')).collection('work').add({
...
db.collection('classes').doc(window.localStorage.getItem('document')).collection('work').orderBy("timestamp").onSnapshot((querySnapshot) => {
    var works = [];
...
function deleteWork(id, email, creatorEmail) {
    ...
    if (email === currentUser.email || currentUser.email === creatorEmail) {
        db.collection('classes').doc(window.localStorage.getItem('document')).collection('work').doc(id).delete()
            .then(function () {
...
function updateChanges() {
    let docRef = db.collection('classes').doc(window.localStorage.getItem('document'))
    let obj = {
...
function cancelChanges() {
    ...
    announcementPermission.checked = Boolean(window.localStorage.getItem('announcement_permission'))
}